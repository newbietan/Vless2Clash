...
function main() {
  ...
  if (!process.env.CLOUDFLARE_API_TOKEN) {
    ...
    console.error(
      "  1. Cloudflare Dashboard → Worker → Settings → Environment Variables",
    );
    console.error(
      "  2. GitHub Actions → Settings → Secrets and variables → Actions",
    );
    console.error("  3. 本地终端: export CLOUDFLARE_API_TOKEN=<your-token>");
    console.error("");
    console.error(
      "创建 Token: https://developers.cloudflare.com/fundamentals/api/get-started/create-token/",
    );
    process.exit(1);
  }
